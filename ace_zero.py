...
def saveArgs(args):
    """
    Saves the argparse namespace to a JSON file, converting Path objects to strings.
    """
    params_dict = vars(args)

    # Create a new dictionary to store JSON-serializable parameters
    serializable_params = {}
    for key, value in params_dict.items():
        if isinstance(value, Path):
            serializable_params[key] = str(value)  # Convert PosixPath to string
        else:
            serializable_params[key] = value

    path = os.path.join(args.results_folder, "args.json")
...
def setupFileLogger(args):
    file_handler = logging.FileHandler(os.path.join(args.results_folder, "ace0.log"))
    file_handler.setLevel(logging.DEBUG)  # Often set to DEBUG to capture all possible logs in the file
...
if __name__ == '__main__':
    ...
    monitor_log = os.path.join(opt.results_folder, f"ace0_monitor_{timestamp}.csv")
    monitor = ProcessMonitor(monitor_log)

    # Start monitoring
    monitor.start_monitoring()
    _logger.info(f"Process monitoring started: {monitor_log}")
    ...
    try:
        ...
        for iteration in range(1, opt.iterations_max):
            ...
            if (registration_rate >= opt.registration_threshold) or (
                (registration_rate - max_registration_rate) < opt.relative_registration_threshold):
                if opt.final_refine:
        ...
        with open(opt.results_folder / "stats.txt", "w") as f:
        ...
        if start and stop is None:
            stop = time.time()
            duration = stop - start
            _logger.info("Reconstruction was interrupted.")

        duration = stop - start if stop else 0.0

        # Stop monitoring and generate summary
        dur = monitor.stop_monitoring()
        _logger.info(f"Monitoring stopped. Monitoring duration: {dur:.2f}s")
